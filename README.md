...
一次性隔离子 Agent 的任务结束时，无修改且无新增提交的 Worktree 会自动删除；存在未提交、未跟踪或新增提交时会保留，并把目录、分支和原因返回主 Agent。该一次性流程不会自动提交、推送、合并或丢弃改动，分支合并仍由上层使用 Git 完成。长期团队代码任务使用下节所述的独立自动集成机制。
...
长期团队的代码任务会自动闭环集成。第一项代码任务领取时，JulyCode 固定记录 Lead 当前命名分支和提交，并从该提交创建本轮专用的内部 Worktree。每项代码任务完成后，系统校验成员 Worktree、任务尝试和结果提交，再以可追踪的 `--no-ff` merge commit 串行加入内部基线；有依赖的成员在领取下游任务前会安全快进到该基线，因此能直接读取上游成果。研究任务不创建空提交。

全部任务成功且内部成果齐全后，完成守卫才会发布。它要求 Lead 仍位于最初记录的分支和提交、工作区包含未跟踪文件在内完全干净、没有正在进行的 Git 操作；随后只执行一次 `--ff-only`，把 Lead 从轮次基线推进到完整的内部结果。发布成功后，空闲且安全的成员会同步到发布提交，内部 Worktree 才会清理。成员 Worktree、团队数据和任务审计记录不会被自动删除。

如果任务合并冲突，系统会中止内部 merge，保留 Lead、成员提交和先前成果，并报告冲突路径及现场目录；它不会猜测或自动解决内容。Lead 变脏、切换分支、HEAD 被外部推进、成员含未登记提交或恢复事实不一致时也会保守拒绝。`integration.json` 按“操作意图 → Git 事实 → 完成记录”保存状态，重启后会核对提交父节点和分支事实，避免重复合并或把未发生的发布误报为成功。

自动流程不执行 push、PR、force、stash、reset、clean、rebase，也不自动处理一次性隔离 subagent 的分支。发布或同步被拒绝后，内部与成员现场会保留，修复安全条件后可以重试。

当前阶段只支持同进程 `coroutine` 成员后端；请求终端窗格等后端会明确失败，不会静默降级。跨机器团队、实时流式成员通信和复杂条件依赖尚未实现。
