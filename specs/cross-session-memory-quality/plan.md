# MewCode 跨会话记忆质量 Plan

## 架构概览
本阶段在现有 `mewcode.memory` 子系统中把自动记忆更新拆成“模型提取候选 → 确定性校验 → 合并或落盘 → 重建索引”四步。模型继续负责理解自然语言和提出候选记忆，确定性校验器负责证据、长期性、关键偏好、敏感信息、重复和冲突门控。这样既保留现有模型提取能力，又把关键偏好高精度要求落实为可测试的程序规则。
...
普通测试和 `--mode offline` 使用确定性 Provider，只证明流程、匹配和统计正确，不声称模型质量。`--mode online` 复用 MewCode 当前配置创建真实 Provider，完整数据集运行后才依据规格阈值给出发布验收结论。
...
所有用例只使用临时目录，不读取当前仓库 `.mewcode/memory`，也不写入真实 `~/.mewcode`。
...
| 在线评测 | 复用当前 Provider 配置并记录元数据 | 结果与实际 MewCode 模型行为一致且可追溯 |
