...
class ScriptedEvalProvider:
    ...
    def _response_for(self, request: ChatRequest) -> ChatMessage:
        ...
        if "你正在为 MewCode 压缩较早的会话历史" in latest_user:
            return ChatMessage(
                role="assistant",
        ...
        if case_id == "basic_qa":
            return ChatMessage(role="assistant", content="MewCode 是一个终端 AI 编程助手，评测重点是可靠、安全、可验证。")
        if case_id == "readonly_search":
            if "read_file" not in tool_names:
                return self._tool("readonly-read", "read_file", {"path": "README.md"})
            return ChatMessage(role="assistant", content="已读取 README。MewCode 的定位是终端 AI 编程助手。")
        if case_id == "multi_tool_loop":
            ...
            if "read_file" not in tool_names:
                return self._tool("multi-read", "read_file", {"path": "src/mewcode/agent.py"})
