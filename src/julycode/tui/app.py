...
from julycode.agent import AgentLoopRunner, TurnEvent
...
from julycode.errors import JulyCodeError, ProviderError, redact_secret
...
from julycode.memory.models import RestoreReport
...
from julycode.providers.base import LLMProvider, TokenUsage
...
from julycode.skills import LOAD_SKILL_TOOL_NAME, LoadSkillTool, SkillConfigurationError, SkillManager, default_skill_roots
from julycode.skills.execution import ProviderResolver, create_isolated_skill_runner, create_provider_resolver
from julycode.subagents import (
...
from julycode.teams import TeamManager, create_team_tools
from julycode.teams.runtime import TeamMemberRunnerFactory, TeamRuntimeSupervisor
from julycode.teams.store import TeamStore
from julycode.worktrees import CleanupReport
...
class JulyCodeApp(App[None]):
    ...
    async def _show_error(self, exc: BaseException) -> None:
        raw = str(exc) if isinstance(exc, JulyCodeError) else f"未预期错误: {exc}"
    ...
    def _print_mcp_warnings(self, report: McpLoadReport) -> None:
        for server_name, error in report.failed_servers.items():
            print(f"JulyCode MCP 警告: Server {server_name} 加载失败: {redact_secret(error)}", file=sys.stderr)
        for tool_name, error in report.failed_tools.items():
            print(f"JulyCode MCP 警告: 工具 {tool_name} 注册失败: {redact_secret(error)}", file=sys.stderr)
        for warning in report.warnings:
            print(f"JulyCode MCP 警告: {redact_secret(warning)}", file=sys.stderr)

    def _report_worktree_cleanup(self, report: CleanupReport) -> None:
        for item in report.failures:
            print(
                f"JulyCode Worktree 清理警告: {item.path}: {redact_secret(item.reason)}",
                file=sys.stderr,
            )
    ...
    async def _refresh_skills(self) -> bool:
        try:
            ...
            return False
        for warning in report.warnings:
            print(f"JulyCode Skill 警告: {warning.source_path}: {warning.message}", file=sys.stderr)
        return True
    ...
    async def _refresh_sub_agents(self) -> bool:
        try:
            ...
            return False
        for warning in report.warnings:
            print(f"JulyCode 子 Agent 警告: {warning.source_path}: {warning.message}", file=sys.stderr)
        return True
