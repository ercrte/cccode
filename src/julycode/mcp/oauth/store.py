...
from julycode.mcp.errors import McpOAuthStorageError
from julycode.mcp.oauth.discovery import normalize_url
from julycode.mcp.oauth.models import OAuthClientRegistration, OAuthCredentialBundle, OAuthTokenSet


KEYRING_SERVICE = "julycode.mcp.oauth"
...
class OAuthCredentialStore:
    ...
    async def _call(self, function: Any, *args: Any) -> Any:
        ...
        # Keyring 后端可能永久阻塞；daemon 线程保证有限等待后不阻塞 JulyCode 退出。
        threading.Thread(target=run, name="julycode-keyring", daemon=True).start()
        try:
