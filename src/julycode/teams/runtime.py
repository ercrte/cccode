...
from julycode.agent import AgentLoopRunner, CompletionDecision
...
from julycode.teams.locking import AtomicJsonFile, FileLock, ProcessLease
...
from julycode.teams.paths import TeamPaths, validate_member_name
from julycode.teams.sessions import TeamMemberSessionStore
from julycode.tools.base import RuntimePrincipal, ToolContext
from julycode.tools.executor import ToolExecutor
from julycode.tools.registry import ToolRegistry
from julycode.worktrees import WorktreeLease, WorktreeManager
...
class TeamRuntimeSupervisor:
    ...
    async def _run_member(self, team_name: str, member: TeamMemberRecord, role: SubAgentRoleDefinition) -> None:
        ...
        session.append_user_message(
            "请处理刚收到的团队消息和当前共享任务。",
            metadata={"julycode_generated": True, "source": "team_runtime"},
        )
        error: str | None = None
        try:
