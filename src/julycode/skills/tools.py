...
from julycode.skills.models import SkillToolDefinition
...
async def _run_blocking(function: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    threading.Thread(target=runner, name="julycode-skill-tool", daemon=True).start()
    while True:
        try:
...
class SkillScriptTool:
    ...
    async def execute(self, arguments: Mapping[str, Any], context: ToolContext) -> Mapping[str, Any]:
        ...
        env = dict(os.environ)
        env["JULYCODE_SKILL_NAME"] = self.skill_name
        env["JULYCODE_SKILL_TOOL"] = self.definition.local_name
        env["JULYCODE_SKILL_DIR"] = str(self.definition.script_path.parent.parent)
        try:
