...
STABLE_PROMPT_MODULES: tuple[PromptBlock, ...] = (
    ...
    PromptBlock(
        ...
        text=(
            ...
            "已知文件或需要查看局部行时使用 read_file，并通过 offset/limit 指定范围；"
            "不知道准确文件名时使用 find_files；需要查找符号、配置、代码或文本时使用 search_code。"
            "只读代码定位和局部查看不得用 run_command 包装 grep、find、sed 或 cat。"
            "编辑前先读取或搜索目标文件；edit_file 的原文必须来自当前文件且唯一匹配。"
            "write_file 会创建或覆盖完整文件，只在需要完整写入时使用。"
            "run_command 会执行本地命令，可能产生副作用；仅在专用工具无法表达任务、"
            "构建、测试、验证或用户明确要求运行命令时使用。"
