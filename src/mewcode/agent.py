...
from mewcode.commands import AgentCommand, AgentMode
from mewcode.config import AgentConfig
from mewcode.context.manager import ContextManager
from mewcode.context.models import ContextCompactionReport, ContextConfig, ContextLimitError
...
from mewcode.subagents.models import ActiveSubAgentPrompt, ParentAgentContext, SubAgentPromptContext, SubAgentToolFilter
...
from mewcode.tools.scheduler import ToolGate
...
class AgentLoopRunner:
    ...
    async def _run_turn(self, command: AgentCommand, *, append_user_message: bool = True) -> AsyncIterator[TurnEvent]:
        ...
        try:
                ...
                if not message.tool_calls:
                    if self.loop_controller is not None:
                        ...
                        if not decision.accept:
                            ...
                            self.session.append_user_message(continuation, metadata={"mewcode_generated": True})
                            continue
                    self.session.append_assistant_message(message)
            ...
            error = str(exc) if isinstance(exc, MewCodeError) else f"未预期错误: {exc}"
