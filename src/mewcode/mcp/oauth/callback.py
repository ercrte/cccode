...
from mewcode.mcp.oauth.models import OAuthCallbackResult
...
class LoopbackOAuthCallback:
    ...
    async def _handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        try:
            ...
            await self._respond(writer, 200, "MewCode OAuth 授权已完成，可以关闭此页面。")
            ...
            await self._respond(writer, 400, "MewCode OAuth 授权失败，请返回终端查看。")
        finally:
            writer.close()
            try:
    ...
    async def _respond(self, writer: asyncio.StreamWriter, status: int, message: str) -> None:
        ...
        body = (
            "<!doctype html><html><head><meta charset='utf-8'><title>MewCode OAuth</title></head>"
